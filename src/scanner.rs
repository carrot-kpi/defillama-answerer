...
pub async fn scan(context: Arc<ChainExecutionContext>) -> anyhow::Result<()> {
    ...
    // When the past indexer is running it has control over the snapshot block number update.
    // This control is passed over to the present indexer only when the past indexer is finished
    // in order to avoid creating holes in the indexing history, In short: the snapshot block number
    ...
    // its share, it communicates it to the present indexer which can then take over the snapshot
    // block number updates.
    let (tx, rx) = oneshot::channel();

    let past_indexing_future =
        past::scan(tx, context.clone()).instrument(tracing::error_span!("past", chain_id));

    let present_indexing_future =
        present::scan(rx, context.clone()).instrument(tracing::error_span!("present", chain_id));
